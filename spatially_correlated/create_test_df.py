...
DATA_ORIGINAL= 'data/in/experimentData1D.csv'
OUTPUT_PATH = 'data/in/test/test_df.csv'
...
def create_test_df(experiment_name: str, output_path: str):
    ...
    ids_in_original = set(df_original['id'].unique())
    # to test id add +1 since id start from 1 in the original data but participant ids in the test set start from 0
    test_participants = {int(pid) + 1 for pid in test_participants}
    df_test = df_original[df_original['id'].isin(test_participants)]
