...
def eval_rw_per_trial(params, df, participant_id=None, N=2, use_simple=False):
    ...
    columns: model_id, trial_index, choice, p_choice, nll.
...
def fit_rw_per_participant(df, participant_col='model_id', N=2):
    """
...
# ---------------------------------------------------------------------------
# Mode 3: eval (fixed true parameters, no fitting)
# ---------------------------------------------------------------------------

def eval_true_params_per_participant(test_df, true_params, param_names=None,
                                     participant_col='participant_id', N=2,
                                     use_simple=True):
    """
    Evaluate model fit using fixed true parameters on each participant's test data.
    No optimisation is performed — true_params are applied as-is.

    Returns
    -------
    summary_df   : one row per participant with NLL and avg NLL/free trial
    all_trial_df : per-trial p_choice and NLL across all participants
    """
    nll_fn = nll_rw_simple if use_simple else nll_rw
    if param_names is None:
        param_names = ['alpha', 'theta', 'd'] if use_simple else \
                      ['alpha_plus', 'alpha_minus', 'a', 'b', 'c', 'd']

    group_cols = ['exp', participant_col] if 'exp' in test_df.columns else [participant_col]
    summary_rows, trial_chunks = [], []

    for key, group in test_df.groupby(group_cols):
        group = group.reset_index(drop=True)
        pid = key if isinstance(key, (int, str, float)) else key[-1]
        val_nll = nll_fn(true_params, group, N=N)
        n_free = (len(group[group['trial_type'] == 'free'])
                  if 'trial_type' in group.columns else len(group))
        avg_nll = val_nll / n_free if n_free else None

        trial_df = eval_rw_per_trial(true_params, group, participant_id=pid,
                                     N=N, use_simple=use_simple)
        trial_chunks.append(trial_df)

        print(f"  participant {pid}: NLL={val_nll:.3f}  |  avg NLL/free trial={avg_nll:.4f}")
        summary_rows.append({
            'participant_id': pid,
            **dict(zip(param_names, true_params)),
            'nll': val_nll,
            'n_free_trials': n_free,
            'avg_nll_per_free_trial': avg_nll,
        })

    summary_df = pd.DataFrame(summary_rows)
    all_trial_df = pd.concat(trial_chunks, ignore_index=True) if trial_chunks else pd.DataFrame()
    return summary_df, all_trial_df


def run_eval(args):
    print('\n=== Mode: eval (true parameters) ===')
    test = normalise_df(pd.read_csv(args.test_csv))
    for _col in ('participant_id', 'participant', 'model_id'):
        if _col in test.columns:
            participant_col = _col
            break
    else:
        raise ValueError("No participant column found (tried: participant_id, participant, model_id)")

    use_simple = getattr(args, 'model', 'simple') == 'simple'
    if use_simple:
        true_params = [args.alpha, args.theta, args.d]
        param_names = ['alpha', 'theta', 'd']
    else:
        true_params = [args.alpha_plus, args.alpha_minus, args.a, args.b, args.c, args.d]
        param_names = ['alpha_plus', 'alpha_minus', 'a', 'b', 'c', 'd']

    print(f'  True params: {dict(zip(param_names, true_params))}')
    print(f'  Test: {len(test)} rows')

    summary_df, trial_df = eval_true_params_per_participant(
        test, true_params, param_names=param_names,
        participant_col=participant_col, use_simple=use_simple,
    )

    total_nll = summary_df['nll'].sum()
    total_free = summary_df['n_free_trials'].sum()
    print(f'\n  Total NLL = {total_nll:.2f}  |  avg NLL/free trial = {total_nll/total_free:.4f}')

    if args.out_summary:
        summary_df.to_csv(args.out_summary, index=False)
        print(f'  Per-participant summary saved → {args.out_summary}')

    if args.out_trial_nll:
        os.makedirs(args.out_trial_nll, exist_ok=True)
        for pid, chunk in trial_df.groupby('participant_id'):
            path = os.path.join(args.out_trial_nll, f'participant_{pid}.csv')
            chunk.to_csv(path, index=False)
        print(f'  Per-trial NLL saved → {args.out_trial_nll}/')
...
def build_parser():
    ...
    # --- eval ---
    e = sub.add_parser('eval', help='Evaluate fit with fixed true parameters (no fitting)')
    e.add_argument('--test-csv',   required=True)
    e.add_argument('--model', choices=['full', 'simple'], default='simple',
                   help='simple (alpha, theta, d) or full (6 params)')
    # simple-model params
    e.add_argument('--alpha',  type=float, default=0.5)
    e.add_argument('--theta',  type=float, default=2.5)
    # full-model params
    e.add_argument('--alpha-plus',  type=float, default=1.0)
    e.add_argument('--alpha-minus', type=float, default=0.3)
    e.add_argument('--a',  type=float, default=5.0)
    e.add_argument('--b',  type=float, default=0.2)
    e.add_argument('--c',  type=float, default=0.05)
    # shared
    e.add_argument('--d',  type=float, default=0.5)
    e.add_argument('--out-summary',   default=None, help='CSV path for per-participant NLL summary')
    e.add_argument('--out-trial-nll', default=None, help='Directory for per-trial NLL CSVs')
